...
local function on_hud_render() -- Handles the HUD layouts 
    ...
    if custom_hud then -- HUD layout
     --   render_lives_segment(22, 15, 1, 1) -- Lives Segment
        --render_coins_segment(halfScreenWidth + 8, 15, 1, 1) -- Coins Segment
        --render_stars_segment(screenWidth - 76, 15, 1, 1) -- Stars Segment
        render_power_meter(10, 5, 65, 65) --scale 65 for default power meter
       -- render_cap_timer(math.ceil(gMarioStates[0].capTimer/30), 22, 35) -- Cap Timer Featured in Some Rom Hacks
        if newTimer then
            render_timer(hud_get_value(HUD_DISPLAY_TIMER), halfScreenWidth - 47, screenHeight * 0.85) --New Timer
        else
